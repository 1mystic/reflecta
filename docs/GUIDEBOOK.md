...
| 13 | Live vitals tick returns **aggregate belief-state only**, and **peeks** (not pops) the pending session | Returning per-item correctness would make `/api/quiz/answer` a cheat oracle for a deliberately server-side answer key; a regression test asserts no `correct_letter` leaks. Peeking (vs popping) leaves the pending session intact so the learner still submits normally. The tick is exempt from the quiz rate-limiter since it is read-only and fires once per answer. |
| 14 | Posterior **variance** returned from the online-IRT fit, not discarded | The Newton step already forms the Hessian; the Laplace variance is `−1/h(θ)`, free. It is the model's uncertainty about the learner (prior at cold start, shrinking with evidence) and drives the live "certainty" signal — the single most compelling vitals readout, for one line of code. |
| 15 | Offline artifacts (`models_store/`, `mlflow.db`) **committed** and shipped in the image | The Model Lab reads them live in prod; without shipping them the deployed app shows "No Models". Un-gitignored (~1.8MB) and re-added to the Dockerfile `COPY`. Subtlety: `.dockerignore` excluded `mlflow.db` *separately* from `.gitignore`, so a git-tracked file still failed `COPY` on the build — both lists had to be reconciled. Everything degrades gracefully when the artifacts are absent (local dev). |
